...
/**
 * Used by *chan_recv* to parse incoming the data. In case of error
 * this function shoul return -2 (-1 is used to indicate an error
 * generated by *chan_recv*
 */
int _recv_ctrl(void *data, size_t size, void *obj);

/**
 * Calleb by chan_send if it is safe to delete the data, that is it
 * has been sent.
 */
void _send_free(void *buffer, void *hint);

void* winienig_wait_for_client(void *_ctx)
{
  using namespace WineingCtrlProto;

  WineingCtx *ctx = (WineingCtx*)_ctx;
  
  log(LOG_INFO, "Synchronizing with client");
  ...
  log(LOG_INFO, "Synchronization terminated "
                "[ctrl_in: %s, ctrl_out: %s, market: %s]",
      ctx->conf->cchan_fqcn,
      ctx->cchan_fqcn_out,
      ctx->conf->mchan_fqcn);
...
void* ctrl_thread(void *_ctx)
{
  ...
  log(LOG_INFO, "Initializing control thread");
  ...
  chan *cchan_in  = chan_init(ctx->conf->cchan_fqcn, CHAN_TYPE_SUB);
  ...
  while (running) {
    ...
    if(rc == 1) {
      ...
      switch(req.type())
        ...
        case Request::INIT:
          err << "Unexpected request type [INIT]!";
          res.set_code(Response::ERR);
          res.set_text(err.str());
          break;
        }
      ...
      google::protobuf::io::ArrayOutputStream os (buffer, 2048);
      res.SerializeToZeroCopyStream(&os);
      chan_send(cchan_out, buffer, res.ByteSize());
...
{
  log(LOG_INFO, "Initializing market data thread");
  ...
  // thread ways after we set the variable.
  // chan *cchan_out = chan_init(ctx->cchan_fqcn_out, CHAN_TYPE_PUSH_CONNECT);
  //chan *mchan  = chan_init(ctx->conf->mchan_fqcn, CHAN_TYPE_PUB);
